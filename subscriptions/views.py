...
class MakePaymentView(APIView):
    ...
    def post(self, request):
        ...
        addons = request.data.get('addons', None)
        price_param = request.data.get('price', None)
        coupon = request.data.get('coupon', None)

        if not subscription_id or not price_param:
            return Response({"error": "subscription_id and price is required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        phone = request.user.mobile_number if request.user.is_authenticated else ""
        surl = f"http://192.168.1.68:8000/payment-success/?s_id={subscription_id}&add_ons={addons}&txn_id={txn_id}&amount={amount}&product_info={product_info}&first_name={first_name}&email={email}&phone={phone}&coupon={coupon}"
        furl = "http://192.168.1.68:8000/payment-failure/"
        ...
        payload = {
            ...
            "furl": furl,
            "hash": hash,
            # "udf1": request.user.id if request.user.is_authenticated else "",
            # "udf2": subscription_id,
            # "udf3": price_param,
        }
